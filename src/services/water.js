...
export const patchWater = async (filter, payload, options = {}) => {
  const rawResult = await WaterCollection.findOneAndUpdate(filter, payload, {
    new: true,
...
export const getWaterConsumptionByDate = async (userId, date) => {
  ...
  const waterDataWithNorma = waterData.map((record) => ({
    ...record.toObject(),
    dailyNorma: userDailyNorma,
  }));
  const totalAmount = waterData.reduce(
...
export const getMonthWater = async (userId, date) => {
  ...
  const totalMonthlyWater = waterMonth.reduce(
    (sum, entry) => sum + (entry.amountOfWater || 0),
    0,
  );
  const daysInMonth = new Date(year, month, 0).getDate();
  const totalMonthlyNorma = daysInMonth * userDailyNorma * 1000;
  ...
  const monthlyPercentage =
    totalMonthlyNorma > 0
      ? Math.floor((totalMonthlyWater / totalMonthlyNorma) * 100)
      : 0;
